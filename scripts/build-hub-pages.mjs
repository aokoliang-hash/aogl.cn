...
const EDITORIAL_I18N = {
  portal: {
    ja: "<p>順序は Similarweb や Cloudflare Radar などの公開トラフィックランキングに沿っています。私個人のクリックログではありません。</p><p>数値は手法・地域でズレます。大きな年次レポートが出たタイミングで見直し、分単位の更新はしません。「もっと見る」は単一の世界前十に入りにくい地域ポータルです。</p>",
    ko: "<p>순서는 Similarweb·Cloudflare Radar 등 공개 글로벌 트래픽 순위를 참고했으며 개인 클릭 통계가 아닙니다.</p><p>지표는 방법론·지역에 따라 어긋날 수 있습니다. 주요 연간 보고가 나올 때 맞추며 실시간 순위를 보장하지 않습니다. ‘더보기’는 단일 글로벌 10위에 넣기 애매한 지역 포털입니다.</p>",
    fr: "<p>L’ordre suit les classements publics de trafic (Similarweb, Cloudflare Radar, etc.) — pas mon journal de clics privé.</p><p>Les classements traînent la réalité (méthodo, géographie). Je réaligne quand les grands rapports annuels sortent, pas minute par minute. « Plus » regroupe des portails régionaux hors d’un seul top 10 global.</p>",
    ru: "<p>Порядок ориентирован на открытые рейтинги трафика (Similarweb, Cloudflare Radar и т.п.) — не на мою личную статистику кликов.</p><p>Таблицы отстают от реальности (метод, география). Подправляю при крупных годовых отчётах, не в реальном времени. В «ещё» — региональные порталы вне одного глобального топ‑10.</p>",
    ar: "<p>يستند الترتيب إلى تصنيفات الزيارات العلنية (Similarweb وCloudflare Radar وغيرها) — وليس إلى سجل نقراتي الشخصي.</p><p>الترتيب يتأخر عن الواقع (المنهجية والجغرافيا). أعيد المطابقة عند التقارير السنوية الكبرى وليس دقيقة بدقيقة. قسم «المزيد» يجمع بوابات إقليمية خارج قائمة عالمية واحدة للعشرة الأوائل.</p>",
  },
  brands: {
    ja: "<p>Interbrand「世界のベストブランド」価値ランキングに近い並びです。年次更新後に順位は変わる場合があります。</p><p>評価額は市場と製品で動きます。ここは<strong>ブックマーク用</strong>であり、投資助言や売買推奨ではありません。</p>",
    ko: "<p>Interbrand식 글로벌 브랜드 가치 상위권과 유사한 순서이며, 연간 발표 후 순위가 바뀔 수 있습니다.</p><p>가치는 시장·제품 주기에 따라 변합니다. 이 페이지는 <strong>북마크 레일</strong>일 뿐 투자 조사나 매매 권유가 아닙니다.</p>",
    fr: "<p>Ordre proche du classement Interbrand « Best Global Brands » ; il peut bouger quand ils publient le tableau annuel.</p><p>La valorisation suit les marchés — ce rail est un <strong>signet</strong>, pas une recherche d’investissement ni un conseil d’achat ou de vente.</p>",
    ru: "<p>Порядок близок к рейтингу Interbrand «Best Global Brands»; после ежегодной публикации он может меняться.</p><p>Оценка пляшет вместе с рынком. Это <strong>закладки</strong>, не инвестиционное исследование и не рекомендация покупать или продавать бумаги.</p>",
    ar: "<p>يقترب الترتيب من ترتيب Interbrand لأفضل العلامات العالمية؛ وقد يتغير عند نشر الجدول السنوي.</p><p>التقييم يتحرك مع الأسواق. الصفحة مجرد <strong>شريط روابط</strong> وليست بحثًا استثماريًا ولا توصية بشراء أو بيع أوراق مالية.</p>",
  },
  shopping: {
    ja: "<p>Digital Commerce 360 や Statista 系の「最大手ネット小売」表に近い順序です。中国国内モールは下の「もっと見る」にまとめています。</p><p>英語圏の「最大」表は米欧本社小売に偏りがちです。中国主導モールを「もっと見る」に分け、同じ見出し同士を比べやすくしています。</p>",
    ko: "<p>Digital Commerce 360·Statista 등 ‘최대 온라인 소매’ 순위와 유사합니다. 중국 전용 몰은 아래 ‘더보기’에 있습니다.</p><p>영어권 ‘최대’ 표는 미·유럽 본사 소매에 치우치기 쉽습니다. 중국 중심 몰을 ‘더보기’로 나눠 같은 헤드라인끼리 비교하기 쉽게 했습니다.</p>",
    fr: "<p>Ordre proche des tableaux « plus grands détaillants en ligne » (Digital Commerce 360, Statista, etc.) — les malls chinois sont dans « plus ».</p><p>Les tableaux « globaux » en anglais penchent vers des sièges US/UE. Les places chinoises vont dans « plus » pour garder une ligne comparable aux titres que vous lisez souvent.</p>",
    ru: "<p>Порядок близок к таблицам «крупнейшие онлайн‑ритейлеры» (Digital Commerce 360, Statista и т.п.) — китайские площадки внизу в «ещё».</p><p>«Глобальные» таблицы на английском чаще про ритейлеров со штабами в США/ЕС. Китайские маркетплейсы в «ещё», чтобы строка совпадала с привычными заголовками.</p>",
    ar: "<p>يقترب الترتيب من جداول «أكبر تجار التجزئة عبر الإنترنت» (Digital Commerce 360 وStatista وغيرهما) — والمراكز الصينية في قسم «المزيد».</p><p>جداول «العالمية» بالإنجليزية تميل لشركات مقرها أمريكا وأوروبا. أسواق الصين في «المزيد» لتسهيل مقارنة الصف مع العناوين التي تقرأها غالبًا.</p>",
  },
  life: {
    ja: "<p>Data.ai / Sensor Tower 系のグローバルMAUチャートに沿った順序です。地域スーパーアプリは下にあります。</p><p>「デジタル生活」は地図・移動・メディア・決済・旅行などの便利ツールまで。遠隔診療や診断ではありません。健康判断は医療専門家と各アプリの注意書きを参照してください。</p>",
    ko: "<p>Data.ai·Sensor Tower 등 글로벌 MAU 차트를 참고했습니다. 지역 슈퍼앱은 아래에 있습니다.</p><p>‘디지털 생활’은 지도·이동·미디어·지갑·여행 편의까지이며 원격 진단이 아닙니다. 건강 결정은 면허 의료인과 각 서비스 안내를 따르세요.</p>",
    fr: "<p>Ordre aligné sur les classements mondiaux de MAU (Data.ai, Sensor Tower, etc.) — les super‑apps régionales sont listées plus bas.</p><p>« Vie numérique » = cartes, trajets, médias, portefeuilles, voyage — pas de télémédecine ni de diagnostic. Pour la santé, parlez à un pro et lisez les mentions légales de chaque app.</p>",
    ru: "<p>Порядок по глобальным рейтингам MAU (Data.ai, Sensor Tower и т.п.) — региональные супер‑приложения ниже.</p><p>«Цифровая жизнь» здесь — карты, такси, медиа, кошельки, путешествия; не телемедицина и не диагноз. За здоровьем — к врачу и в официальные тексты сервисов.</p>",
    ar: "<p>يستند الترتيب إلى تصنيفات المستخدمين النشطين عالميًا (Data.ai وSensor Tower وغيرهما) — والتطبيقات الإقليمية الكبرى في الأسفل.</p><p>«الحياة الرقمية» هنا تعني الخرائط والتنقل والوسائط والمحافظ والسفر — وليست تشخيصًا طبيًا عن بُعد. للصحة راجع مختصًا مرخصًا ونصوص كل تطبيق.</p>",
  },
  social: {
    ja: "<p>Data.ai / We Are Social 系のグローバルSNS MAUランキングに近い順序です。いずれの製品政策を推奨するものではありません。</p><p>MAUはニュースサイクルより遅れます。国内閉じ・超ニッチはノイズ削減のため省きます。Hot mixはRSSでタイトル英語のまま出ることがあります。</p>",
    ko: "<p>Data.ai·We Are Social 등 글로벌 소셜 MAU 순위와 유사합니다. 특정 정책을 지지하는 뜻은 아닙니다.</p><p>MAU는 보도 주기보다 늦습니다. 국내 폐쇄·초소형 네트워크는 가독성을 위해 생략했습니다. Hot mix는 RSS이며 제목이 영어로 남을 수 있습니다.</p>",
    fr: "<p>Ordre proche des classements mondiaux d’apps sociales (Data.ai, We Are Social, etc.) — sans endosser les politiques produit.</p><p>Les MAU traînent les cycles presse ; messagers nationaux et niches minuscules sont omis pour garder la ligne lisible. Le hot mix RSS garde souvent l’anglais à la source.</p>",
    ru: "<p>Порядок близок к глобальным рейтингам соцприложений (Data.ai, We Are Social и т.д.) — без одобрения политик продуктов.</p><p>MAU отстают от прессы; локальные мессенджеры и крошечные ниши убраны ради читаемости. Hot mix из RSS — заголовки часто остаются на английском.</p>",
    ar: "<p>يقترب الترتيب من تصنيفات تطبيقات التواصل العالمية (Data.ai وWe Are Social وغيرهما) — دون تأييد سياسات أي منتج.</p><p>أرقام المستخدمين النشطين تتأخر عن دورة الأخبار؛ حذفت شبكات محلية ضيقة لتقليل الضجيج. خليط RSS أدناه يبقي العناوين إنجليزية غالبًا.</p>",
  },
  tech: {
    ja: "<p>NVIDIA・TSMC・Broadcom など公開の時価ランキングに近い並びです。投資推奨ではありません。</p><p>portal の大衆サイト流量とは違い、ここはCAPEX/GPU供給の話に出る半導体・プラットフォーム寄りです。提出書類と自分の許容リスクは必ず各自で確認してください。</p>",
    ko: "<p>NVIDIA·TSMC·Broadcom 등 공개 시가총액 순위와 유사합니다. 매수 권유가 아닙니다.</p><p>portal의 대중 웹 트래픽과 달리 여기는 반도체·플랫폼의 CAPEX/GPU 서사에 가깝습니다. 여전히 투자 조언이 아니며 공시와 본인의 위험 감수를 확인하세요.</p>",
    fr: "<p>Ordre proche des tableaux de capitalisation (NVIDIA, TSMC, Broadcom, etc.) — pas une liste d’achat.</p><p>Contrairement au portail (trafic grand public), ici ce sont semi‑conducteurs et hyperscalers des cycles GPU/capex — toujours <strong>pas</strong> un conseil en investissement.</p>",
    ru: "<p>Порядок близок к таблицам капитализации (NVIDIA, TSMC, Broadcom и т.д.) — не список для покупки.</p><p>В отличие от портала (массовый веб‑трафик), здесь полупроводники и гиперскейлеры из историй про capex/GPU — по‑прежнему <strong>не</strong> инвестиционный совет.</p>",
    ar: "<p>يقترب الترتيب من جداول القيمة السوقية (NVIDIA وTSMC وBroadcom وغيرها) — وليس قائمة شراء.</p><p>بخلاف صفحة البوابة (زيارات الويب الجماهيرية)، هنا شرائح ومنصات تظهر في روايات نفقات رأس المال ووحدات GPU — ما زال <strong>ليس</strong> نصيحة استثمارية.</p>",
  },
  games: {
    ja: "<p>PC・リビング支出では Steam と御三家が先行。Tencent 級のモバイル巨人は主站の形が違うため「もっと見る」に置いています。</p><p>Hot mix は npm run fetch-hub-news で公開RSSを結合。重複除去はベストエフォート、日付はフィード基準、見出しは英語のまま出ることがあります。</p>",
    ko: "<p>PC·거실 지출은 Steam과 콘솔 3사가 앞섭니다. 텐센트급 모바일 거인은 웹 허브 형태가 달라 ‘더보기’에 두었습니다.</p><p>Hot mix는 npm run fetch-hub-news로 공개 RSS 병합, 중복 제거는 최선 노력, 날짜는 피드 기준이며 제목은 영어로 남을 수 있습니다.</p>",
    fr: "<p>Steam et les trois familles de consoles dominent PC / salon ; les géants mobiles type Tencent sont dans « plus » car leurs hubs web diffèrent.</p><p>Le hot mix fusionne les RSS publics via npm run fetch-hub-news — dédup partiel, dates des flux, titres souvent en anglais à la source.</p>",
    ru: "<p>Steam и три консольные семьи лидируют в PC / гостиной; мобильные гиганты уровня Tencent — в «ещё», так как их веб‑хабы иные.</p><p>Hot mix собирает публичные RSS через npm run fetch-hub-news — дедуп приблизительный, даты из лент, заголовки часто на английском.</p>",
    ar: "<p>يغلب Steam وعائلات المنصات الثلاث إنفاق الحاسوب والصالة؛ عملاقو الموبايل على غرار Tencent في قسم «المزيد» لأن مراكزهم الويب تختلف.</p><p>يمزج Hot mix خلاصات RSS العامة عبر npm run fetch-hub-news — إزالة التكرار تقريبية والتواريخ من التغذيات والعناوين غالبًا بالإنجليزية.</p>",
  },
  tools: {
    ja: "<p>天気・翻訳・電卓などは各サービスの公式ページへリンクしています。利用規約・データの扱いは必ず各サイトで確認してください。</p><p>大規模なSaaS目録ではなく、日常でよく使う少数のショートカットです。aogl.cn 上で処理は行わずリンクのみです。</p>",
    ko: "<p>날씨·번역·계산기 등은 각 서비스 공식 페이지로 연결됩니다. 약관과 데이터 처리는 해당 사이트에서 확인하세요.</p><p>모든 SaaS 목록이 아니라 자주 쓰는 소수 바로가기입니다. aogl.cn에서는 실행 없이 링크만 제공합니다.</p>",
    fr: "<p>Liens vers les services officiels (météo, traduction, calculatrice, etc.). Vérifiez toujours leurs conditions et leur traitement des données.</p><p>Rail court pour usages quotidiens — pas un annuaire géant. Rien ne s’exécute sur aogl.cn hors liens sortants.</p>",
    ru: "<p>Ссылки ведут на официальные сервисы (погода, перевод, калькулятор и т.д.). Условия и данные — на сторонних сайтах.</p><p>Короткий набор частых утилит — не каталог всех SaaS. На aogl.cn ничего не выполняется, только внешние ссылки.</p>",
    ar: "<p>روابط إلى صفحات الخدمات الرسمية (الطقس، الترجمة، الحاسبة، إلخ). راجع شروط كل موقع ومعالجة البيانات هناك.</p><p>قائمة قصيرة للأدوات اليومية — ليس دليل كل خدمات SaaS. لا يُشغَّل شيء على aogl.cn سوى الروابط الخارجية.</p>",
  },
};
